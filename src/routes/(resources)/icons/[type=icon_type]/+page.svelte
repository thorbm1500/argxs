...
<script lang="ts">
	...
	import { cubicOut, quartInOut, quintOut } from 'svelte/easing';
	...
	const allFilters: { id: number, tag: string, active: boolean }[] = $state([]);
	...
	let icons: ResourceIcon[] = $derived.by(() => {
		...
		const filters = allFilters.filter(t => t.active);
		const hasAllActiveFilters = (tags: string[]) => {
		...
		if (filters.length) {
			...
			for (const icon of result) {
				if (hasAllActiveFilters(icon.tags) || hasAllActiveFilters(icon.default.tags)) {
					newResult.push(icon);
				}
				else {
					for (const v of icon.variable) {
						if (hasAllActiveFilters(v.tags)) {
							newResult.push(icon);
							break;
						}
					}
		...
		document?.addEventListener('click', (event) => {
			...
			if (iconContainerOpened !== null) {
				...
				while (el !== null) {
					...
					if (/(overlay buttons)|(highlighted-icon)|(element (prev|next)-icon)|(light-button)|(header-section)/.test(el.className)) return;
	...
	<div class="highlighted-icon {backgroundLight ? 'lights-on' : 'lights-off'}" transition:fade={{duration: prefersReducedMotion ? 0 : 100}}>
		...
		<div class="h-icon">
			{#key hCurrentIcon}
				...
				<button title="Turn the lights {backgroundLight ? 'off' : 'on'}" class="light-button" onclick={() => {
				...
				<div class="left">
							...
							<div in:fade|global={{duration: prefersReducedMotion.current ? 0 : 200}} class="icon-meta">
								...
								<div class="meta icon-source">
									{#if highlightedIcon.iconIndex[highlightedIcon.currentIcon]?.source}
										<p>Sourced from</p>
										<a href={highlightedIcon.iconIndex[highlightedIcon.currentIcon]?.source?.href} rel="external" target="_blank" tabindex="15" style="font-weight: 550;">
					...
					{#if highlightedIcon.iconIndex.length > 1}
						...
						<div class="actions">
							<button title="Previous" class="element prev-icon" tabindex="2" onclick="{() => {
									currentSVG = '';
									if (!highlightedIcon) return;
							...
							<button title="Next" class="element next-icon" tabindex="2" onclick="{() => {
									currentSVG = '';
									if (!highlightedIcon) return;
				...
				<div class="right">
					...
					<div class="actions" tabindex="-1">
						<p class="version-tag" id="version-tag" tabindex="-1">Added in Version {highlightedIcon.iconIndex[highlightedIcon.currentIcon]?.version}</p>
					...
					<div class="tags">
						{#each hCurrentIcon.tags as tag}
							<button class="tag" onclick="{() => allFilters.forEach(t => t.active = tag === t.tag)}">#{tag}</button>
						{/each}
					</div>
				</div>
			{/key}
			...
			<GlassButton className="search-field">
				<form class="search-field" onsubmit="{(e) => {
					...
					<input type="search" id="icon-search-field" placeholder="Search..." bind:value={searchbarValue}
					       onfocusout="{() => search = searchbarValue}"
					       oninput="{() => {
				...
				<div class="sorting">
					...
					<GlassButton className="sort-action">
						<button type="button" class="sort-action" onclick="{() =>  {
							...
							else if (order === 'desc') sorting = 'default';
							else order = 'desc';
							saveCurrentSorting();
					}}">
							<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    .meta {
                        ...
                        &.icon-source {
                            ...
                            * {
                            ...
                            a:hover {
                                color: light-dark(var(--theme-text-primary), var(--theme-text-secondary));
                            }
                        }
                    }
                }
            ...
            .right {
                ...
                .actions {
                    ...
                    .version-tag {
                        ...
                        color: oklch(from var(--theme-text-fourth) l c h / .75);
                        font-size: .725rem;
                        font-weight: 550;
