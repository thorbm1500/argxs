...
<section class='home-page-sec'>
	<div
		style='transform-style: preserve-3d; transform: translateY({Math.max(minY, Math.min(maxY, (coords.current.y / 750) - .6))}rem) translateX({Math.max(minX, Math.min(maxX, (coords.current.x / 750) - 1.1))}rem) rotate3d(-1,0,0, -15deg) rotate3d(0,1,0, {$state.eager(deg)}deg)'>
...
<style>
	    ...
	    @media (width >= 44rem) {
		    .home-page-sec {
			    left:  25vw;
			    width: calc(50vw - var(--sidebar-width));
		    }
	    }
